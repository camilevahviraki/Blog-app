...
RSpec.describe "User_show", type: :feature do
  ...
  subject { User.new(Name: 'camilux', Photo: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSQx5pB6py4twIFGBfFT4IS8E13iYj_pzGyKw&usqp=CAU', Bio: 'This is the corresponding wireframe view', PostsCounter: 3)} 
    before :each do
     subject.save
     Post.create(Title: 'Test title1', Text: 'Test Text1', CommentsCounter: 0, LikesCounter: 0, author_id: User.find(subject.id).id)
    ...
    it "When I click a user's post, it redirects me to that post's show page." do
      ...
      expect(page).to have_current_path("/users/#{User.find(subject.id).id}/posts/#{Post.find_by(Title: 'Test title1').id}")
    end
